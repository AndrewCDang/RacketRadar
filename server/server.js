    ...
    if (ipRequestCounts.has(ip)) {
        ...
        if (requestCountData[racketId]._id.length >= 2) {
            ...
            return res.status(429).json({
                error: "Too many of the same requests. (≖_≖ ) Try again later.",
                type: "limit",
            });
        }
        ...
        console.log(
            `Request count for ${racketId}: ${requestCountData[racketId].count}/n ip:${requestCountData[racketId].ip}`
        );
    } else {
        ipRequestCounts.set(ip, {
            [racketId]: { count: 1, ip: [ip], _id: [userId] },
        });
    }
...
const selfPing = () => {
    axios
        .get(`https://racket-radar.netlify.app/`)
        .then((response) => {
