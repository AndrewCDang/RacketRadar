...
const StickySearch = ({searchBarRef, NONE, searchRef, handleSearch, racketsRef, searchValue }) => {

    const [state, dispatch] = useContext(RacketContext)
    const displayMenu = state.menuSelection.searchMenu ? {display: 'block'} : {display: 'none'}
    ...
    const deleteRacket = (id) => {
        const racketListId = state.racketList_id
        console.log(racketListId)
        console.log(id)
    ...
    useEffect(()=>{
        ...
        }else if (state.selectedRackets.objects.length==0 && state.racketList.length>0){
            let matchedIndexes = []
    ...
    return(
        <aside ref={searchBarRef} style={displayMenu} className="sticky-interact-1">
            <section ref={searchRef} className="search-menu">
                <input ref={inputRef} onChange={handleSearch} className='searchBar' id='searchInput' placeholder="RacketName"></input>
                <div className="close-btn" onClick={(e)=>{dispatch({type:'menuSelection', payload: {searchMenu: false,filterMenu: false,addMenu: false}}); inputRef.current.value = ''; showAll()}}>
                    <svg fill="#000000" className='close' width="26px" height="26px" viewBox="-8.5 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
                        <title>close</title>
