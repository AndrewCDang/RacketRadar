...
function Landing() {
    ...
    useEffect(() => {}, [state.selectedRackets.objects]);
    ...
    useEffect(() => {
                ...
                return (
                    <div
                        ...
                        style={{
                            ...
                            transform:
                                scaleIcon[index] || buttonActive[index]
                                    ? "scale(1)"
                                    : "scale(0)",
                            ...
                            zIndex:
                                buttonActive[index] && defZIndex == index
                                    ? state.racketList.length + index
                                    : defZIndex[index],
                        }}
                    >
                        <div key={index} className="graph-racket-dot">
                            ...
                            <div className="graph-dot-bg graph-def">
                                ...
                                <div
                                    style={{
                                        backgroundImage: `radial-gradient(at 25% 100%, hsla(183,95%,43%,1) 0px, transparent ${
                                            racket.balance - 20
                                        }%), 
                                    radial-gradient(at 85% 25%, hsla(97,77%,60%,1) 0px, transparent ${
                                        100 - racket.stiff - 20
                                    }%), 
                                    radial-gradient(at 35% 20%, hsla(43,100%,66%,1) 0px, transparent ${
                                        racket.stiff - 20
                                    }%), 
                                    radial-gradient(at 75% 100%, hsla(347,100%,66%,1) 0px, transparent ${
                                        100 - racket.balance - 10
                                    }%)`,
                                        transition: "all 0.3s ease-in-out",
                                        opacity: buttonActive[index] ? 1 : 0,
                                        border: buttonActive[index]
                                            ? "3px solid white"
                                            : "unset",
                                    }}
                                    className={`icon-dot-bg ${
                                        buttonActive[index]
                                            ? "icon-dot-ani"
                                            : ""
                                    }`}
                        ...
                        <div
                            className="dot-btns"
                            style={NONE}
                            ref={(el) => (dotBtnRef.current[index] = el)}
                        >
                            <a
                                className="dot-btn-a"
                                onClick={() => viewRacket(racket._id)}
                            >
                                <svg
                                    className="svg-compare"
                                    viewBox="0 0 80 80"
                                    fill="none"
                                ...
                                <span>
                                    {" "}
                                    {buttonActive[index]
                                        ? "Remove racket details"
                                        : "View racket details"}{" "}
                                </span>
                            </a>
                            <a
                                className="dot-btn-a"
                                ref={(el) => (dotBack.current[index] = el)}
                                onClick={(e) => sendBack(e, index)}
                            >
                                <svg
                                    className="svg-back"
                                    data-name="Layer 1"
                                    xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 390.2 266.2"
                                >
        ...
        buttonActive,
        state.selectedRackets.objects,
        state.selectedRackets.deleteId,
        displayIcon,
        scaleIcon,
        forceUpdate,
        focusIndex,
    ...
    const handleClick = useCallback((el, e, index) => {
        ...
        switch (styleBtn) {
            case "none":
                ...
                if (
                    dotBtnRef.current[index].offsetWidth + e.clientX >
                    window.innerWidth
                ) {
                    dotBtnRef.current[index].style.left = "-8px";
                    dotBtnRef.current[
                        index
                    ].style.transform = `translateY(-50%) translateX(-${dotBtnRef.current[index].offsetWidth}px)`;
                } else {
            ...
            {racketObjects ? (
                <article className="graph">
                    ...
                    <section className="graph-container">
                        ...
                        <div className="heavy">
                            H<span>ead</span>-Heavy
                        </div>
                        <div className="light">
                            H<span>ead</span>-Light
                        </div>
