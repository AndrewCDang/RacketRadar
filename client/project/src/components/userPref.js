...
function UserPref() {
    ...
    const [diffState, setDiffState] = useState({
        ...
        singleDoubles: [null, null, null],
        rely: [null, null, null],
        speedPower: [null, null, null, null, null],
        generationControl: [null, null, null, null, null],
        fitness: [null, null, null, null, null],
    });
    ...
    useEffect(() => {
        if (
            diffState.level[0] !== null &&
            diffState.style[0] !== null &&
            diffState.singleDoubles[0] !== null &&
            diffState.rely[0] !== null &&
            diffState.speedPower[0] !== null &&
            diffState.generationControl[0] !== null &&
            diffState.fitness[0] !== null
        ) {
            let level;
            let style;
            let singleDoubles;
            ...
            const userFlex =
                (level.flex + style.flex + singleDoubles.flex) / 3 +
                rely.flex +
                speedPower.flex +
                generationControl.flex +
                fitness.flex;
            const userbalance =
                (level.balance + style.balance + singleDoubles.balance) / 3 +
                rely.balance +
                speedPower.balance +
                generationControl.balance +
                fitness.balance;

            setUserStats({
                stiff: Math.round(userFlex),
                balance: Math.round(userbalance),
            });
            dispatch({
                type: "userStats",
                payload: {
                    flex: Math.round(userFlex),
                    balance: Math.round(userbalance),
                },
            });
        }
    ...
    const speedPowerHandler = (index) => {
        setDiffState((prevState) => ({
            ...prevState,
            speedPower: prevState.speedPower.map((item, idx) =>
                idx === index ? false : true
            ),
        }));
    };

    const generationControlLabels = [
        ["Max PG", "Low Control"],
        ["More PG", "Less Control"],
        ["Balanced PG", "Balanced Control"],
        ["More Control", "Less PG"],
        ["Max Control", "Low PG"],
    ];

    const generationControlHandler = (index) => {
        setDiffState((prevState) => ({
            ...prevState,
            generationControl: prevState.generationControl.map((item, idx) =>
                idx === index ? false : true
            ),
        }));
    };
    ...
    const fitnessHandler = (index) => {
        setDiffState((prevState) => ({
            ...prevState,
            fitness: prevState.fitness.map((item, idx) =>
                idx === index ? false : true
            ),
        }));
    };
    ...
    return (
        <div
            ...
            style={{
                ...
                display: "grid",
                gridTemplateRows: displayMenu ? "1fr" : "0fr",
            }}
            className="userPref-container"
        >
            <div style={{ position: "relative" }}>
                ...
                <p className="pref-title-main">
                    Find out what racket specs suits you
                </p>
                <p className="pref-title">
                    <span>What is your</span> skill level<span>? [ 1/7 ]</span>
                </p>

                <div className="style-container">
                    <div
                        ref={(el) => (diffRef.current[0] = el)}
                        style={{
                            backgroundColor: diffState.level[0]
                                ? "rgba(70, 70, 70, 0.9)"
                                : null,
                            flex:
                                diffState.level[0] == false &&
                                stateHover.level == false
                                    ? 4
                                    : null,
                        }}
                        onClick={() =>
                    ...
                    >
                        <div
                            style={{ opacity: diffState.level[0] ? 0.5 : null }}
                            className="media-height user-style-card"
                        >
                            <svg
                                className="svg-badminton-2"
                                data-name="Layer 1"
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 402 265.35"
                            >
                            ...
                            <div className="user-style">Beginner</div>
                        </div>
                        <div className="style-card-text">
                            <ul>
                                <li className="user-card-text">
                                    Basic Technique (Grip, Footwork, Strokes)
                                </li>
                                <li className="user-card-text">
                                ...
                                <li className="user-card-text">
                                    Control (Clears, Smash, Front-line serves )
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div
                        ref={(el) => (diffRef.current[1] = el)}
                        style={{
                            backgroundColor: diffState.level[1]
                                ? "rgba(70, 70, 70, 0.9)"
                                : null,
                            flex:
                                diffState.level[1] == false &&
                                stateHover.level == false
                                    ? 4
                                    : null,
                        }}
                    ...
                    >
                        <div
                            style={{ opacity: diffState.level[1] ? 0.5 : null }}
                            className="media-height user-style-card"
                        >
                            <svg
                                className="svg-badminton-2"
                            ...
                            <div className="user-style">Intermediate</div>
                        </div>
                        <div className="style-card-text">
                            <ul>
                                <li className="user-card-text">
                                    Advanced Technique (Refined Strokes)
                                </li>
                                <li className="user-card-text">
                                    Tactical Awareness
                                </li>
                                ...
                                <li className="user-card-text">
                                    Shot Variety (Drop Shots, Drives,
                                    Cross-court Shots)
                                </li>
                                <li className="user-card-text">
                                    Control (Slices, Netplay )
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div
                        ref={(el) => (diffRef.current[2] = el)}
                        style={{
                            backgroundColor: diffState.level[2]
                                ? "rgba(70, 70, 70, 0.9)"
                                : null,
                            flex:
                                diffState.level[2] == false &&
                                stateHover.level == false
                                    ? 4
                                    : null,
                        }}
                    ...
                    >
                        <div
                            style={{ opacity: diffState.level[2] ? 0.5 : null }}
                            className="media-height user-style-card"
                        >
                            <svg
                                className="svg-badminton-2"
                            ...
                            <div className="user-style">Advanced</div>
                        </div>
                        <div className="style-card-text">
                            <ul>
                                <li className="user-card-text">
                                    Advanced Tactics (Exploiting Weaknesses,
                                ...
                                <li className="user-card-text">
                                    Deception and Skill Variation (Deceptive
                                    Shots)
                                </li>
                                <li className="user-card-text">
                ...
                <p className="pref-title">
                    Aggressive <span>or</span> Balanced <span>or</span>{" "}
                    Defensive<span> style? [ 2/7 ]</span>
                </p>

                <div className="style-container">
                    <div
                        ref={(el) => (diffRef.current[3] = el)}
                        style={{
                            backgroundColor: diffState.style[0]
                                ? "rgba(70, 70, 70, 0.9)"
                                : null,
                            flex:
                                diffState.style[0] == false &&
                                stateHover.style == false
                                    ? 4
                                    : null,
                        }}
                        onClick={() =>
                    ...
                    >
                        <div
                            style={{ opacity: diffState.style[0] ? 0.5 : null }}
                            className="media-height user-style-card"
                        >
                            <svg
                                className="svg-badminton"
                                data-name="Layer 1"
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 712.88 1231.71"
                            >
                            ...
                            <div className="user-style">Aggressive</div>
                        </div>
                        <div className="style-card-text user-card-text">
                            An aggressive player in badminton is someone who
                            focuses on attacking and putting pressure on their
                            opponents. They tend to play powerful shots, such as
                            smashes and aggressive clears, to force their
                            opponents into defensive positions.{" "}
                        </div>
                    </div>
                    <div
                        ref={(el) => (diffRef.current[4] = el)}
                        style={{
                            backgroundColor: diffState.style[1]
                                ? "rgba(70, 70, 70, 0.9)"
                                : null,
                            flex:
                                diffState.style[1] == false &&
                                stateHover.style == false
                                    ? 4
                                    : null,
                        }}
                    ...
                    >
                        <div
                            style={{ opacity: diffState.style[1] ? 0.5 : null }}
                            className="media-height user-style-card"
                        >
                            <svg
                                className="svg-badminton"
                            ...
                            <div className="user-style">Balanced</div>
                        </div>
                        <div className="style-card-text user-card-text">
                            A balanced player in badminton refers to someone who
                            combines elements of both aggressive and defensive
                            playstyles. They possess a versatile approach to the
                            game, adapting their strategies based on the
                            situation and the opponent they are facing
                        </div>
                    </div>
                    <div
                        ref={(el) => (diffRef.current[5] = el)}
                        style={{
                            backgroundColor: diffState.style[2]
                                ? "rgba(70, 70, 70, 0.9)"
                                : null,
                            flex:
                                diffState.style[2] == false &&
                                stateHover.style == false
                                    ? 5
                                    : null,
                        }}
                    ...
                    >
                        <div
                            style={{ opacity: diffState.style[2] ? 0.5 : null }}
                            className="media-height user-style-card"
                        >
                            <svg
                                className="svg-badminton"
                            ...
                            <div className="user-style">Defensive</div>
                        </div>
                        <div className="style-card-text user-card-text">
                            A defensive player in badminton emphasises strong
                            defensive skills and the ability to retrieve shots
                            effectively. They prioritize maintaining good court
                            coverage, returning opponents' shots with accuracy
                            and precision.
                        </div>
                    </div>
                </div>
                ...
                <p className="pref-title">
                    {" "}
                    <span>Do you play </span>Singles <span>or</span>Doubles
                    <span>? [ 3/7 ]</span>
                </p>

                <div className="style-container">
                    <div
                        ref={(el) => (diffRef.current[6] = el)}
                        style={{
                            backgroundColor: diffState.singleDoubles[0]
                                ? "rgba(70, 70, 70, 0.9)"
                                : null,
                            flex:
                                diffState.singleDoubles[0] == false &&
                                stateHover.singleDoubles == false
                                    ? 4
                                    : null,
                        }}
                        onClick={() =>
                    ...
                    >
                        <div
                            style={{
                                opacity: diffState.singleDoubles[0]
                                    ? 0.5
                                    : null,
                            }}
                            className="media-height user-style-card"
                        >
                            ...
                            <div className="user-style">Singles Player</div>
                        </div>
                    </div>
                    <div
                        ref={(el) => (diffRef.current[7] = el)}
                        style={{
                            backgroundColor: diffState.singleDoubles[1]
                                ? "rgba(70, 70, 70, 0.9)"
                                : null,
                            flex:
                                diffState.singleDoubles[1] == false &&
                                stateHover.singleDoubles == false
                                    ? 4
                                    : null,
                        }}
                    ...
                    >
                        <div
                            style={{
                                opacity: diffState.singleDoubles[1]
                                    ? 0.5
                                    : null,
                            }}
                            className="media-height user-style-card"
                    ...
                    <div
                        ref={(el) => (diffRef.current[8] = el)}
                        style={{
                            backgroundColor: diffState.singleDoubles[2]
                                ? "rgba(70, 70, 70, 0.9)"
                                : null,
                            flex:
                                diffState.singleDoubles[2] == false &&
                                stateHover.singleDoubles == false
                                    ? 4
                                    : null,
                        }}
                    ...
                    >
                        <div
                            style={{
                                opacity: diffState.singleDoubles[2]
                                    ? 0.5
                                    : null,
                            }}
                            className="media-height user-style-card"
                        >
                            ...
                            <div className="user-style">Doubles Player</div>
                        </div>
                    </div>
                </div>

                <p className="pref-title">
                    <span>Which of these shots do your </span>rely{" "}
                    <span>on the most? [ 4/7 ]</span>
                </p>

                <div className="style-container">
                    <div
                        ref={(el) => (diffRef.current[9] = el)}
                        style={{
                            backgroundColor: diffState.rely[0]
                                ? "rgba(70, 70, 70, 0.9)"
                                : null,
                            flex:
                                diffState.rely[0] == false &&
                                stateHover.rely == false
                                    ? 4
                                    : null,
                        }}
                        onClick={() =>
                            setDiffState({
                                ...diffState,
                                rely: [false, true, true],
                            })
                        }
                        className="style-grid-2"
                    >
                        <div
                            style={{ opacity: diffState.rely[0] ? 0.5 : null }}
                            className="media-height user-style-card"
                        >
                            <svg
                                id="Layer_1"
                            ...
                            >
                                ...
                                <polyline
                                    className="smash-2"
                                    points="324.76 157.09 357.01 224.37 285.12 204.39"
                                />
                                <line
                                    className="smash-2"
                                    x1="354.74"
                            ...
                            <div className="user-style">Powerful Smashes</div>
                        </div>
                    </div>
                    <div
                        ref={(el) => (diffRef.current[10] = el)}
                        style={{
                            backgroundColor: diffState.rely[1]
                                ? "rgba(70, 70, 70, 0.9)"
                                : null,
                            flex:
                                diffState.rely[1] == false &&
                                stateHover.rely == false
                                    ? 4
                                    : null,
                        }}
                        onClick={() =>
                            setDiffState({
                                ...diffState,
                                rely: [true, false, true],
                            })
                        }
                        className="style-grid-2"
                    >
                        <div
                            style={{ opacity: diffState.rely[1] ? 0.5 : null }}
                            className="media-height user-style-card"
                        >
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 426.76 259.98"
                            >
                                ...
                                <g id="Layer_4" data-name="Layer 4">
                                    <path
                                        className="drop-2"
                                        d="M250.51,199.08C250.51,111.53,149.79,37.73,12.39,15.12"
                                    />
                                    <polyline
                                        className="drop-2"
                                        points="277.4 147.15 252.62 217.52 215.93 152.56"
                                    />
                                </g>
                            </svg>
                            <div className="user-style">
                    ...
                    <div
                        ref={(el) => (diffRef.current[11] = el)}
                        style={{
                            backgroundColor: diffState.rely[2]
                                ? "rgba(70, 70, 70, 0.9)"
                                : null,
                            flex:
                                diffState.rely[2] == false &&
                                stateHover.rely == false
                                    ? 4
                                    : null,
                        }}
                        onClick={() =>
                            setDiffState({
                                ...diffState,
                                rely: [true, true, false],
                            })
                        }
                        className="style-grid-2"
                    >
                        <div
                            style={{ opacity: diffState.rely[2] ? 0.5 : null }}
                            className="media-height user-style-card"
                        >
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                            ...
                            >
                                ...
                                <g id="Layer_3" data-name="Layer 3">
                                    <path
                                        className="clear-2"
                                        d="M391.94,199.08c0-105.53-85.55-191.08-191.08-191.08S9.78,93.55,9.78,199.08"
                                    />
                                    <polyline
                                        className="clear-2"
                                        points="419.22 147.15 394.44 217.52 357.74 152.56"
                                    />
                                </g>
                            </svg>
                            <div className="user-style">Defensive clears</div>
                        </div>
                    </div>
                </div>

                <p className="pref-title">
                    Racket-Speed <span>vs</span> Racket-Power{" "}
                    <span> [ 5/7 ]</span>
                </p>
                <Slider
                    id="drag-sp"
                    labels={speedPowerLabels}
                    handler={speedPowerHandler}
                    selectedBtn={diffState.speedPower}
                />

                <p className="pref-title">
                    Power-Generation(PG) <span>vs</span> Control
                    <span> [ 6/7 ]</span>
                </p>

                <Slider
                    id="drag-gc"
                    labels={generationControlLabels}
                    handler={generationControlHandler}
                    selectedBtn={diffState.generationControl}
                />

                <p className="pref-title">
                    Fitness Level <span>[ 7/7 ]</span>
                </p>
                <Slider
                    id="drag-f"
                    labels={fitnessLabel}
                    handler={fitnessHandler}
                    selectedBtn={diffState.fitness}
                />
