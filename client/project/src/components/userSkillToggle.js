    ...
    return(
        <article className='toggle-btns-container'>
            <div style={{zIndex:5}} className='ust-container'>
                <div className='graphToggle-btn' onClick={() => !userToggle ? toggleUserPref() : dispatch({ type: 'userRacket', payload: { userRacket: false }})} style={{border: '1px solid white', borderRadius: '12px',width:`${!userToggle ? widthToggle + 32 + 48 : widthToggle + 32}px`, transition:"width 0.3s ease", overflow:'hidden', padding:"8px 0px", display:"flex", placeSelf:'center'}}>
                    {!userToggle ? <svg className='user-dot-svg-toggle' width="40" height="40" viewBox="0 0 70 70" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div ref={btnRef} className={`ust-btn ${userToggle ? 'ust-expanded' : ''}`} >{!userToggle ? ` ${userSet ? 'Edit' : 'Place'} user on graph` : "Close"}</div>
                </div>
                <div className='ust-subText'>{ !userToggle ? 'Set user racket skills/preferences' : <>&nbsp;</>}</div>
            </div>
            <div style={{zIndex:5}} className='ust-container'>
                <div className='graphToggle-btn' onClick={() => !filterToggle ? toggleFilter() : dispatch({ type: 'filterSelection', payload: { filterMenu: false }})} style={{border: '1px solid white', borderRadius: '12px',width:`${!filterToggle ? widthFilterToggle + 32 + 40 : widthFilterToggle + 32}px`, transition:"width 0.3s ease", overflow:'hidden', padding:"8px 0px", display:"flex", placeSelf:'center'}}>
                    {!filterToggle ? <svg className='user-filter-svg-toggle' width="40" height="40" viewBox="-5.5 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
                    <title>filter</title>
                    ...
                    <div ref={filterRef} className={`ust-btn ${filterToggle ? 'ust-expanded' : ''}`} >{!filterToggle ? "Filter Rackets" : "Close"}</div>
                </div>
                <div className='ust-subText'>{ !filterToggle ? 'Filter via price, difficulty, and more  ' : <>&nbsp;</>}</div>
            </div>
        </article>
    )
