...
function App() {
    ...
    const reducer = (state, action) => {
        switch (action.type) {
            ...
            case "deleteId":
                const newIdList = state.selectedRackets.objects.filter(
                    (id) => id !== action.payload.deleteId
                );
            ...
            case "favRackets":
                if (!state.favRackets.includes(action.payload)) {
                    ...
                    return {
                        ...state,
                        favRackets: [...state.favRackets, action.payload],
                    };
                } else {
            ...
            case "wishRackets":
                if (!state.wishRackets.includes(action.payload)) {
                    ...
                    return {
                        ...state,
                        wishRackets: [...state.wishRackets, action.payload],
                    };
                } else {
            ...
            case "selectedRacketsFromNav":
                return {
                    ...
                    selectedRackets: {
                        condition: false,
                        objects: [...state.selectedRackets.objects],
                    },
                };
    ...
    const [state, dispatch] = useReducer(reducer, {
        ...
        menuSelection: { searchMenu: false },
        filterSelection: { filterMenu: false },
        userRacket: { userRacket: false },
        favouriteToggle: false,
        wishToggle: false,
        selectedRackets: { condition: false, objects: [] },
        deleteId: { deletedId: "", deletedFrom: "" },
        userStats: { flex: "", balance: "" },
        favRackets: [],
        wishRackets: [],
        cardAnimation: true,
        navUnderline: { width: 0, start: 0 },
        clientNotification: "",
        scrolledFirstItem: false,
    });
